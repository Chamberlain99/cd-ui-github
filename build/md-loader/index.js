/**
 * @description index.js文件是loader的入口文件，通过提取template 与 script 的内容，把 Markdown 转化成 Vue 组件。
 * 
 */
...
module.exports = function (source) {
  ...
  // 注释Tag 开始结束的名称和长度
  const startTag = '<!--cdui-demo:';
  const startTagLen = startTag.length;
  const endTag = ':cdui-demo-->';
  ...
  while (commentStart !== -1 && commentEnd !== -1) {
    ...
    const demoComponentName = `cdui-demo${id}`; // 内联组件名称
